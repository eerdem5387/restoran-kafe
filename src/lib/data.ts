...
function toDbCategory(category: MenuCategory): PrismaMenuCategory {
  return category.replace(/-/g, "_") as PrismaMenuCategory;
}

function toAppCategory(category: PrismaMenuCategory): MenuCategory {
  return category.replace(/_/g, "-") as MenuCategory;
}
...
function mapMenuItem(item: PrismaMenuItem): MenuItem {
  return {
    ...
    category: toAppCategory(item.category),
    tags: item.tags,
...
export async function getMenuItemById(id: string): Promise<MenuItem | undefined> {
  ...
  const item = await getPrisma().menuItem.findUnique({ where: { id } });
  ...
  try {
    const item = await getPrisma().menuItem.update({
      ...
      data: {
        ...
        ...(updates.category !== undefined && { category: toDbCategory(updates.category) }),
