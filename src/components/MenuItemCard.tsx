...
export function MenuItemCard({
  ...
  const localized = localizedMenuItem(item, locale);
  ...
  const content = (
    <>
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-center text-start">
        ...
        <MenuTags tags={localized.tags} className="mt-2" />
      </div>
    </>
  );
  ...
  if (onSelect) {
    return (
      <motion.button
        ...
        className="soft-shadow group flex w-full gap-3 overflow-hidden rounded bg-surface-container-lowest p-3 text-start transition-shadow duration-500 hover:shadow-[0_12px_40px_rgba(75,54,33,0.1)] sm:gap-4 sm:p-4"
      >
        {content}
      </motion.button>
    );
...
export function MenuItemRow({
  ...
  return (
    <motion.button
      ...
      variants={fadeUp}
      onClick={() => onSelect?.(item)}
      className="group flex w-full items-start gap-3 text-start sm:gap-4 md:gap-5"
      whileHover={{ x: locale === "ar" ? -3 : 3, transition: { duration: 0.35, ease: [0.22, 1, 0.36, 1] } }}
      whileTap={{ scale: 0.99 }}
    >
      <ProductThumb
        src={item.image}
      ...
      <div className="min-w-0 flex-1 pt-0.5">
        ...
        <MenuTags tags={localized.tags} light={light} className="mt-2" />
      </div>
    </motion.button>
  );
}
